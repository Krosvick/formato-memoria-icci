...
#let page-number(
  ...
  util.maybe-locate(loc, loc => {
    ...
    if passthrough {
      ...
      let result = if type(num) == function or (type(num) == str and util.cardinality(num) == 2) {
...
///   document, the numberings can be a `str`, `function` or `none`
/// - alignments (array): an array of alignments for each part ("left", "center", "right")
...
#let anti-matter(
  ...
  let valid-alignments = (left, center, right)
  for a in alignments {
    assert(a in valid-alignments, message: "Alignment must be valid")
  }

  set page(
    header: step() + header,
    numbering: (..args) => locate(loc => {
      let key = core.part(loc)
      let (_, idx) = core.select(key)
      let alignment = core.numbering-state().at(loc).alignments.at(idx)
